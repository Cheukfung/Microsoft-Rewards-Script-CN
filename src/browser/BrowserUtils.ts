...
export default class BrowserUtils {
    ...
    async tryDismissAllMessages(page: Page): Promise<void> {
        try {
            ...
            if (overlay) {
                ...
                if (rejected) {
                    this.bot.logger.debug(this.bot.isMobile, 'DISMISS-ALL-MESSAGES', '已关闭：Bing 叠加层拒绝')
                } else {
    ...
    async getLatestTab(page: Page): Promise<Page> {
        try {
            ...
            if (!newTab) {
                this.bot.logger.error(this.bot.isMobile, 'GET-NEW-TAB', '未找到任何标签页！')
    ...
    async closeTabs(page: Page, config = { minTabs: 1, maxTabs: 1 }): Promise<Page> {
        try {
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'SEARCH-CLOSE-TABS',
                `检测到 ${tabs.length} 个已打开的标签页（最少：${config.minTabs}，最多：${config.maxTabs}）`
            )
    ...
    async ghostClick(page: Page, selector: string, options?: ClickOptions): Promise<boolean> {
        try {
            ...
            this.bot.logger.error(
                this.bot.isMobile,
                'GHOST-CLICK',
                `选择器 ${selector} 操作失败：${error instanceof Error ? error.message : String(error)}`
            )
            return false
        }
    }
        ...
        await page.route(routePattern, route => {
            try {
                ...
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'DISABLE-FIDO',
                    `已修改请求体：isFidoSupported 设置为 ${body.isFidoSupported}`
                )
