    ...
    async getDashboardData(): Promise<DashboardData> {
        try {
            ...
            this.bot.logger.info(
                this.bot.isMobile,
                'GET-DASHBOARD-DATA',
                `获取仪表盘数据出错：${error instanceof Error ? error.message : String(error)}`
            )
            throw error
        }
    }
    ...
    async getAppDashboardData(): Promise<AppDashboardData> {
        try {
            ...
            this.bot.logger.info(
                ...
                `获取应用仪表盘数据出错：${error instanceof Error ? error.message : String(error)}`
            )
            throw error
        }
    }
    ...
    async getXBoxDashboardData(): Promise<XboxDashboardData> {
        try {
            ...
            this.bot.logger.info(
                ...
                `获取 Xbox 仪表盘数据出错：${error instanceof Error ? error.message : String(error)}`
            )
            throw error
        }
    }
    ...
    async closeBrowser(browser: BrowserContext, email: string) {
        try {
            ...
            this.bot.logger.debug(
                this.bot.isMobile,
                'CLOSE-BROWSER',
                `正在将 ${cookies.length} 个 Cookie 保存到会话目录！`
            )
            ...
            // Close browser
            await browser.close()
            this.bot.logger.info(this.bot.isMobile, 'CLOSE-BROWSER', '浏览器已正常关闭！')
        } catch (error) {
            this.bot.logger.error(
                this.bot.isMobile,
                'CLOSE-BROWSER',
