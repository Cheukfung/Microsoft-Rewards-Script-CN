...
export class MobileAccessLogin {
    ...
    async get(email: string): Promise<string> {
        try {
            ...
            this.bot.logger.info(this.bot.isMobile, 'LOGIN-APP', '正在等待移动端 OAuth 授权码...')
            ...
            if (!code) {
                this.bot.logger.warn(this.bot.isMobile, 'LOGIN-APP', '等待 OAuth 授权码超时')
                return ''
            }
            ...
            if (!token) {
                this.bot.logger.warn(this.bot.isMobile, 'LOGIN-APP', '令牌响应中缺少 access_token')
                return ''
            }

            this.bot.logger.info(this.bot.isMobile, 'LOGIN-APP', '已获取移动端访问令牌')
            return token
        } catch (error) {
            this.bot.logger.error(
                this.bot.isMobile,
                'LOGIN-APP',
                `移动端访问错误：${error instanceof Error ? error.message : String(error)}`
            )
            return ''
        } finally {
